---
title: Excalidraw Release Notes
source: https://excalidraw-obsidian.online/WIKI/08%20Release%20Notes/Excalidraw%20Release%20Notes.md
---
...
%%
```js
let list = (await app.vault.read(app.vault.getFileByPath('WIKI/08 Release Notes/Excalidraw Release Notes.md'))).split("## Releases\n")[1].trim();
res = "";
for(rel of list.replaceAll("- [[","").replaceAll("]]\n", ",").split(",").filter(r=>r.match(/\d+\.\d+\.\d+$/))) {
    const f = app.vault.getFileByPath("WIKI/08 Release Notes/releases/"+rel+".md");
    const txt = await app.vault.read(f);
    res += "# "+rel+"\n\n"+txt+"\n";
}
res = res.replaceAll(/\((https:\/\/youtu\.be\/[^)]+)\)/g,"($1)\n$1\n");
app.vault.create("Release Notes.md", res);
```
%%
